...
* Rework the `Error` type, remove several `Error::is_*` methods. ([a230d3c][a230d3c])

* Replace `DownloadAction::Url` with `DownloadAction::FileObj`. ([1fd5ff6][1fd5ff6], [8fbd8d1][8fbd8d1])
...
* Remove `Users::get|list|iter` methods. The `/users` endpoints are no longer supported. ([1c547aa][1c547aa])

* Replace list & iter methods with `search(filter)` returning the `Query<T>` type which implements
  various methods to load search results. ([ebf5374][ebf5374], [f8a35de][f8a35de])
...
* Add expiry date of the access token. ([9445c3c][9445c3c])

* Remove all deprecated code. ([c3032af][c3032af])
